...
export default function UpdateAddressModal({
  ...
  const [shortAddress, setShortAddress] = useState<boolean | null>(null);
  ...
  useEffect(() => {
    if (isOpen && address) {
      reset({
        ...
        company_name: (address?.company_name as string) || "",
        email: (address?.email as string) || "",
        mobile_number: (address?.mobile_number as string) || "",
        city: (address?.city as string) || "",
        address_line_1:
          (address?.full_address as string) ||
          (address?.address_line_1 as string) ||
          "",
        sa_short_address: (address?.sa_short_address as string) || "",
        address_id: String(address?.id || ""),
      });

      // Set shortAddress state based on whether address has sa_short_address
      if (
        address?.sa_short_address &&
        String(address.sa_short_address).length === 8
      ) {
      ...
      // Load city from address if it exists
      const loadCityFromAddress = async () => {
        const cityName = (address?.city as string) || "";
        if (cityName) {
          const existingCity = await checkCityExists(cityName);
          if (existingCity) {
  ...
  async function getShortAddress(signal: AbortSignal) {
    ...
    try {
      ...
      if (response?.data?.success) {
        ...
        if (data) {
          setShortAddress(data as unknown as boolean);
      ...
      (responseData) => {
        ...
        if (returnWizardOpened) {
          if (addressInsertionType === "shipper") {
            dispatch(
              _insertReturnAddress([{ ...addressData, selected: true }])
            );
                  ...
                  <Input
                    ...
                    {...register("sa_short_address", {
                      ...
                      validate: (value) => {
                        ...
                        if (value.length !== 8)
                          return t("nationalAddressLength");
