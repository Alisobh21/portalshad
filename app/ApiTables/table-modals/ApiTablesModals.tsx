...
import { DatalistModal, DatalistObjModal, HTMLParsedModal } from "./ColumnsStaticModals";
import { ConfirmationModal, ViewRowData } from "./RowActionsModals";
import { useDispatch, useSelector } from "react-redux";
import { _getClickedRowAction, _getClickedRowActionResponse, _getCustomControlRequest } from "../table-providers/slices/rowActionsSlice";
...
function ApiTablesModals() {
    const { selectedBulkAction } = useSelector((state: any) => state?.bulkActions);
    const { rowSelectedModal } = useSelector((state: any) => state?.tableColumns);
    const { clickedRowAction, clickedRowActionResponse, customControlAction } = useSelector((state: any) => state?.rowActions);
    ...
    return (
        <>
            ...
            <Popup status={clickedRowActionResponse} closeModal={handleCloseModal} isVisible={!!clickedRowActionResponse}>
            ...
            <Popup
                status={clickedRowAction?.need_confirmation || selectedBulkAction?.need_confirmation}
                closeModal={handleCloseModal}
                containerClass="max-w-xl"
                isVisible={!!clickedRowAction?.need_confirmation || !!selectedBulkAction?.need_confirmation}
            >
                <ConfirmationModal
                    closeModal={handleCloseModal}
                    confirmationFor={clickedRowAction ? "rowAction" : selectedBulkAction && "bulkAction"}
                />
            </Popup>

            {/* Static Columns Modal - Displays column data in various formats */}
            <Popup status={rowSelectedModal} containerClass="max-w-lg" closeModal={handleCloseStaticModals} isVisible={!!rowSelectedModal}>
                {Array.isArray(rowSelectedModal?.value?.value) ? (
                    <DatalistModal closeModal={handleCloseStaticModals} />
                ) : rowSelectedModal?.value?.value && Array.isArray(Object.keys(rowSelectedModal?.value?.value)) ? (
            ...
            <Popup status={customControlAction} closeModal={handleCloseModal} containerClass={`max-w-2xl`} isVisible={!!customControlAction}>
                {customControlAction?.url?.api?.includes("link-product-to-available-slot") && (
                    <LinkProductWithHubSlot action={customControlAction} closeModal={handleCloseModal} />
                )}
                {customControlAction?.url?.api?.includes("link-available-slot-to-product") && (
                    <LinkSlotToProduct action={customControlAction} closeModal={handleCloseModal} />
                )}
                {customControlAction?.url?.api?.includes("update-tote") && <ModifyTote action={customControlAction} closeModal={handleCloseModal} />}
                {customControlAction?.url?.api?.includes("assign-picker-to-order") && (
                    <AssignOrderToPicker action={customControlAction} closeModal={handleCloseModal} />
                )}
            </Popup>
        </>
    );
