...
import { objectToArrayKeyVal, restructureSelectedFilters } from "../table-utils/utils";
...
function TableFilters({ showFilters, setShowFilters }: { showFilters: boolean; setShowFilters: (showFilters: boolean) => void }) {
    const { tableName, structureFilters } = useSelector((state: RootState) => state.tableCore);
    ...
    useEffect(() => {
        const isAnyFieldFilled = Object.values(watchFields).some(
            (field: any) => field?.fieldValue && (typeof field.fieldValue === "string" && field.fieldValue?.trim()) !== ""
        );
    ...
    function submitFiltersHandler(data: Record<string, any>) {
        const updatedForTags = Object.fromEntries(
            Object.entries(data).map(([key, value]: [string, any]) => {
                if (typeof value.fieldValue === "string" && value.operator === "in") {
                    return [key, { ...value, fieldValue: value.fieldValue.split(",")?.filter((el: string) => el !== "") }];
                } else if (value.operator === "between" && typeof value.fieldValue === "object") {
                    return [key, { ...value, fieldValue: formatDatesRange(value.fieldValue)?.value }];
                }
                return [key, value];
            })
        );
        const updatedForDate = Object.fromEntries(
            Object.entries(data).map(([key, value]: [string, any]) => {
                if (value.operator === "between" && typeof value.fieldValue === "object") {
                    return [key, { ...value, fieldValue: formatDatesRange(value.fieldValue)?.label }];
                }
                return [key, value];
            })
        );
        ...
        <>
            {structureFilters?.length > 0 && (
                <Sheet open={showFilters} onOpenChange={setShowFilters}>
                    ...
                    <SheetContent className="z-[999] rounded-xl overflow-hidden! pb-5">
                        <SheetHeader>
                        ...
                        <div className="col absolute bottom-0 p-3 bg-background  left-0 right-0">
                            <Button form="tb_filters_form" type="submit" variant="primary-opac" className="px-5 w-full" disabled={!isSubmitEnabled}>
                                {t("apply")}
                            </Button>
                        </div>
                    </SheetContent>
