...
import { getIntersectedRowActions, objectToArrayValue } from "../table-utils/utils";
...
function TableRowActions({ row, col }: any) {
    ...
    const { actionsInRegularCells } = useSelector((state: any) => state?.rowActions);
    ...
    useEffect(() => {
        if (selectedRows?.length > 0) {
            dispatch(_getSelectedRowActions(objectToArrayValue(getIntersectedRowActions(selectedRows?.map((row: any) => row?.actions)))));
        } else {
        ...
        <>
            {rowActions?.filter((action) => action)?.length > 0 ? (
                <Popover>
                    ...
                    <PopoverContent align="end" className="inline-flex min-w-[160px] p-2">
