...
export default function ShipmentStep() {
  ...
  async function submitShipmentStep() {
    ...
    if (isValid) {
      dispatch(
        _setShipmentStepValue({
          pickup: shippingType,
          payment_type: watch("payment_type") ? "cod" : "cc",
          ...
          ...(watch("price") && watch("payment_type")
            ? { price: watch("price") }
            : {}),
        })
      );
  ...
  async function getCarriersList() {
    ...
    try {
      ...
      const response = await axiosPrivate.get<CarriersResponse>(
        `/carrier-services/filtered-by-city?from=${shipperCity}&to=${consigneeCity}&shipment_direction=shipping`
      );
  ...
  useEffect(() => {
    dispatch(
      _setShipmentStepValue({ ...shippmentStepValues, pickup: shippingType })
    );
    getCarriersList();
  ...
  const selectedCarrier = carriersList?.find(
    (carrier) => carrier?.slug === watch("shipping_company")
  );
      ...
      <div className={cn("w-full", wizardCurrentStep === 3 ? "" : "hidden")}>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
          ...
          {!getCarriersLoading && carriersList?.length === 0 ? (
            <div className="px-4 py-2 lg:col-span-2 rounded-lg bg-neutral-200/20 dark:bg-neutral-700/20">
          ...
          {watch("shipping_company") && (
            <div className="flex flex-col gap-2 lg:col-span-2">
              {selectedCarrier?.cod_available ? (
                <div className="flex items-center gap-2">
                  <Checkbox
                    id="payment_type"
                    checked={watch("payment_type") || false}
                    onCheckedChange={(checked) => {
                      setValue("payment_type", checked as boolean);
                    }}
                  />
                  <Label
                ...
                <div className="flex flex-col items-start justify-start gap-1">
                  <div className="flex items-center gap-2 opacity-70 pointer-events-none">
                    <Checkbox id="payment_type_disabled" checked={false} />
                    <Label
                      htmlFor="payment_type_disabled"
                      className="text-sm font-normal"
                    >
                      {t("cashOnDelivery")}
                    </Label>
                  </div>
                  <p className="text-sm text-muted-foreground">{t("noCOD")}</p>
                </div>
              )}
            </div>
          )}
            ...
            <div className="flex flex-col lg:col-span-2 gap-2">
              ...
              <Input
                ...
                {...register("price", {
                  required: {
                    value: true,
                    message: `${t("pleaseEnter")} ${t("pricePlaceholder")}`,
                  },
                })}
                value={watch("price") || ""}
                onChange={(e) => {
