  ...
  async function createShippingAirwaybill() {
    ...
    try {
        ...
        `/awb-processing/return/store`,
        {
          ...
          receiving_addr:
            consigneeStepValues?.return_dest_type !== "ss-repository"
              ? consigneeStepValues?.consignee_address_id
              : "",
          ...
          order_number:
            (orderToBeReturned as string) || shippmentStepValues?.order_number,
          ...
          return_dest_type: consigneeStepValues?.return_dest_type,
        }
      );
      ...
      if (response?.data?.success) {
        ...
        dispatch(_getUserReturnAddresses([]));
        ...
        dispatch(_setDefConsigneeOpt("warehouse"));
        setTimeout(() => {
          router.back();
        }, 3000);
      }
      ...
      console.error("Error creating return AWB:", err);
      setCreateError("Failed to create return AWB");
    } finally {
