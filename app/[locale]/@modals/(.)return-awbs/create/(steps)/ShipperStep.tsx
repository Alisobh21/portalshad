...
export default function ShipperStep() {
  const { returnAddresses, loaders } = useSelector(
    (state: RootState) => state.geolocation
  );
  const { wizardCurrentStep, shipperStepValues, returnAdditionalData } =
  ...
  useEffect(() => {
    if (returnAddresses?.length > 0) {
      if (!shipperStepValues?.shipper_address_id) {
        const addressToBeSelected =
          (returnAddresses as AddressWithDetails[])?.find(
            (address) => address?.selected
          ) || returnAddresses?.[0];
        setSelectedAddress(addressToBeSelected as AddressWithDetails);
        setValue("shipper_address_id", addressToBeSelected?.id);
        setValue("shipper_name", addressToBeSelected?.name || "");
        setValue("shipper_company", addressToBeSelected?.company_name || "");
        setValue("shipper_phone", addressToBeSelected?.mobile_number || "");
        ...
        const savedAddress =
          ...
          ({
            ...(shipperStepValues?.shipper_full_address as AddressWithDetails),
            mobile_number:
              watch("shipper_phone") ||
              (shipperStepValues?.shipper_full_address as AddressWithDetails)
                ?.mobile_number,
        ...
        if (savedAddress) {
          ...
          setValue("shipper_address_id", savedAddress?.id);
          setValue("shipper_name", savedAddress?.name || "");
          setValue("shipper_company", savedAddress?.company_name || "");
          setValue("shipper_phone", savedAddress?.mobile_number || "");
        }
      }
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [returnAddresses, shipperStepValues?.shipper_address_id]);
  ...
  const filteredAddresses = (returnAddresses as AddressWithDetails[])
      ...
      <div className={cn("w-full", wizardCurrentStep === 1 ? "" : "hidden")}>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
          ...
          <div className="flex flex-col gap-2 lg:col-span-2">
            ...
            {/* Return Additional Data Alert */}
            {returnAdditionalData &&
            typeof returnAdditionalData === "string" ? (
              <Alert variant="warning" className="lg:col-span-2">
                <AlertDescription>
